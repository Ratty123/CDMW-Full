...
- Removed the Studio's stage walls, so the view no longer changes as you orbit or zoom. Walls cannot be made to hold still: any wall between the camera and the character has to be hidden or it paints over the subject, so which walls exist depends on where you are looking. Ranking the four by distance and drawing the farthest two popped as the ranking reshuffled; drawing every wall the camera faces put the nearest one across the whole viewport as a flat slab. The stage is now a grid floor with a bright boundary edge — the same geometry from every angle, changing only by perspective, which is what Blender does and what this stage was asked to be compared against.
