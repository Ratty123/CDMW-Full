...
| Format decode progress | `schemas/archive_content_capabilities.v1.json` is the single source of truth for what each file format can be read and written as; `tools/report_format_decode_progress.py` validates it and regenerates the derived summary and report | `schemas/archive_extension_inventory.json` holds the shipped build's extension counts and is what keeps the manifest honest about what the game actually contains; `tools/dotnet_archive_backend/src/Cdmw.Archive.Content/ArchiveContentRegistry.cs` embeds and reads the manifest; `cdmw/domain/archives/content_capabilities.py` is the Python reader, and `cdmw/domain/archives/association_vocabulary.py` derives the Associated Assets reference pattern, companion suffixes, and group order from it so archive linking cannot drift from what the manifest registers | `tests/test_format_decode_progress.py`, `tests/test_archive_wwise_bank_and_association_vocabulary.py` | `docs/features/format-decode-progress.md` |
...
| Translation Studio (`.paloc` editing) | `tools/translation_studio/catalogue.py` domain, `language_index.py` cached package lookup, `table_model.py` virtualised model over 187,521 rows, `tab.py` panel, `ai_provider.py`/`ai_translate.py`/`ai_job.py`/`ai_panel.py` bring-your-own-key machine translation | registered as a lazy tool tab in `cdmw/ui/shell/tool_tabs.py`; `cdmw/core/paloc_format.py` owns the format | `tests/test_translation_studio.py` (synthetic plus a `real_game` gate) | `docs/features/translation-studio.md` |
...
| Projectile packages (`.paprojdesc`) | `cdmw/core/projectile_package.py` reads and rebuilds the index that says which projectile-info modules each package loads | archive extraction for the shipped file | `tests/test_projectile_package.py`, including a `real_game` gate that checks the decode against the XML it was compiled from | — |
| Constraint rigs (`.papr`) | `cdmw/core/papr_format.py` structural reader, byte-exact writer, and the weight/transform/rename editors; `cdmw/core/papr_block.py` decodes the configuration block itself (driver lists, limits, and the 3ds Max expression controllers that carry the actual rule) | `tools/placement_studio/constraints.py` groups entries into chains and builds the mod payload; `tools/placement_studio/window_constraints.py` is the Secondary motion panel; `cdmw/core/archive_binary_preview_papr_0.py` for the preview-pane scrape | `tests/test_papr_format.py`, `tests/test_papr_block.py` and `tests/test_placement_studio_constraints.py` (synthetic plus `real_game` corpus gates) | `docs/features/localization-and-constraint-formats.md` |
...
| Prefab structural decoding | `cdmw/core/prefab_binary.py` decoder with `prefab_collection_spans.py` span types and `prefab_blob_tail.py` close rules, `prefab_binary_edit.py` path/placement writers and `prefab_array_edit.py` collection resizer; `prefab_asset_catalog.py` archive path index; `cdmw/domain/archives/prefab_{glossary,values,companions}.py`; `cdmw/services/prefab_structure_service.py` facade | Prefab Inspector (`cdmw/ui/archive_browser/prefab_inspector_{dialog,actions,structure}.py`); loose mod export path | `tests/test_prefab_binary.py`, `tests/test_prefab_binary_edit.py`, `tests/test_prefab_array_edit.py`, `tests/test_prefab_values.py`, `tests/test_prefab_glossary.py`, `tests/test_prefab_companions.py`, `tests/test_prefab_asset_catalog.py`, `tests/test_prefab_inspector_dialog.py`, `tests/test_prefab_inspector_structure.py` | `docs/features/prefab-structural-decoding.md` |
...
| Mesh Editor and replacement builder | `cdmw/ui/mesh_editor/tab.py` and `workspace.py` public classes over bounded `tab_*.py` and `workspace_*.py` UI owners; `cdmw/ui/preview/` splits host protocol/value handling and session localization behind stable host/session classes; thin static-replacement callback/section facades over bounded `static_replacement_dialog_{callbacks,sections}_*_part_*.py` owners; thin mesh-edit factory over bounded `static_replacement_mesh_edit_*.py` state/action/history/stroke owners; `cdmw/ui/archive_browser/mesh_launch_flow.py` | `cdmw/services/mesh_service.py` facade plus focused service owners, including native-session serialization and native-clone geometry; `mesh_workflow_service.py`; bounded `cdmw/modding/mesh_native_*.py` client/session/payload/kernel/report owners behind `mesh_native_core.py`; `cdmw/modding/static_mesh_*.py`; compatibility-only offline package owners under `cdmw/rendering/native_preview_*.py`; resident `native/cdmw_mesh_core/`; sole production .NET/Vortice renderer split across partial-class owners under `tools/dotnet_mesh_editor_experiment/`; `tools/mesh_harness/` splits input, zoom, performance, flow, Qt probes, and service coverage behind `tools/mesh_editor_dev_harness.py`; mesh schemas | decomposition gates plus focused moved-owner guards: `tests/test_mesh_editor_tab_decomposition.py`, `tests/test_static_replacement_mesh_edit_decomposition.py`, `tests/test_mesh_service_decomposition.py`, `tests/test_dotnet_preview_shared_host*.py`, `tests/test_dotnet_mesh_editor_*source.py`, `tests/test_mesh_edit_*source_guard*.py`, `tests/test_mesh_harness_*.py`, `tests/test_mesh_*.py`, and `tests/test_static_replacement_*.py` | `docs/features/mesh-editing-pipeline.md`, `docs/features/mesh-editor-visual-material-parity-audit.md`, `docs/mesh_editor_net_repair_audit.md`, `docs/mesh_editor_net_authoritative_renderer_audit.md` |
...
| Interface localization | `cdmw/domain/localization.py`, packaged catalogs under `cdmw/resources/localization/`, `cdmw/services/localization_file_service.py`, `cdmw/services/startup_localization_service.py`, `cdmw/workers/localization_workers.py` | process-scoped `cdmw/ui/localization.py`, shell language/theme/startup controllers, startup host, resident Python/.NET preview session, WinForms `ExperimentForm.UiLocalization.cs` | `tests/test_localization_async_io.py`, `tests/test_localization_translations.py`, `tests/test_localization_catalog_contracts.py`, `tests/test_dotnet_ui_localization_protocol_source.py`, `tests/test_dotnet_preview_shared_host.py` | `docs/features/interface-localization.md` |
