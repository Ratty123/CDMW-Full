...
| Constraint rigs (`.papr`) | `cdmw/core/papr_format.py` header reader, weight locator, and verified in-place weight editor | `cdmw/core/archive_binary_preview_papr_0.py` for the expression/reference scrape in the preview pane | `tests/test_papr_format.py` (synthetic plus a `real_game` corpus gate) | `docs/features/localization-and-constraint-formats.md` |
