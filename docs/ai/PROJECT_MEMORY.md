...
- Release Python dependencies are pinned by tested constraints. CI installs .NET 10,
  builds the exact helper, runs nonvisual Python 3.11/3.14 gates, and gates packaging on both QA jobs. Headless report tests launch the built Windows-subsystem apphost directly; never use `dotnet run` as their completion fence because SDK versions may return before a detached WinExe writes its report.
