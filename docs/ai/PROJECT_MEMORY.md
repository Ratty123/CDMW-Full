...
- Release Python dependencies are pinned by tested constraints. CI installs .NET 10,
  builds the exact helper, runs nonvisual Python 3.11/3.14 gates, and gates packaging on both QA jobs. Headless report tests invoke the built DLL through the `dotnet` console host, retain its temp directory until terminal JSON parses, and reuse that report across parameterized consumers; process lifetime and status are not completion fences for a WinExe across SDK versions. Treat the JSON report as authoritative.
