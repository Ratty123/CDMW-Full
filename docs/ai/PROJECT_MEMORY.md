...
- `scripts/codex_check.ps1 -Area mesh-unit` is nonvisual mesh coverage. It owns the Builder unresolved-global audit and offscreen Import Mesh/Modify Original construction gate; release packaging repeats the synthetic Builder target without renderer, archive, or licensed-asset I/O. Source-string guards alone do not close escaped runtime wiring regressions. `tools/mesh_editor_dev_harness.py --scenario headless-edit-mesh-diagnostics` is the composed hidden Solid (Textured)/all-tools synthetic gate: it runs the production Vortice form, Python dispatcher, native C++ edit and Morph/Refit sessions, inventories every declared control, and semantically checks all selection, transform, topology, part, layer, Colour, Morph/Refit, display, and camera families without licensed data. Move and Grab each use 320-update held streams and require bounded coalescing, an idle dispatcher, exact final native authority, and zero snap-back. Native Select All and Invert must enumerate every editable session submesh rather than deriving target parts from the incoming selection. The gate emits a build log, protocol/report JSON, session timeline, result, and background interaction JSONL. The host interaction trail must use `MeshInteractionFlightRecorder`; producers only enqueue shallow correlated events, while its bounded daemon owns path resolution, serialization, batching, and disk writes. Modify Original's original-texture callback factory must install `_original_reference_texture_preview_set_native_package_path_helper`, `_apply_native_preview_core_material_manifest_helper`, and `_native_manifest_input_from_descriptor`; resolver diagnostics retain bounded transitions plus request, stage, worker/thread state, and traceback.
...
- External material factors are immutable per synthesized texture: normalize and scan material parameters once, then apply the resolved factors per pixel. Bounded float32 NumPy row chunks preserve byte-identical roughness/metalness/specular outputs and cancellation checks; exact cold `cd_pgm_00_nude_00_0001.pac` material synthesis fell from 5.103 to 1.812 seconds after the earlier 93.214-second baseline.
...
- Embedded Edit Mesh consumes Escape instead of rejecting the whole builder; explicit close/replacement still works. Its status and widened FPS readout live in the pane headers, its camera strip remains inside the resident preview, compact 8.5-point tool panels replace the oversized side controls, and embedded status/footer rows consume no preview height. Embedded startup constructs the authoring panels and permanently parents rail-only sections while the child is hidden; reveal sizes the native child first, then presents and shows it, and Edit Mesh moves only the two sections genuinely shared with placement. Archive Browser remains active during static-replacement construction: the Qt builder mounts and shows under its hidden parent, closes startup progress, and only then queues one Mesh Editor activation; embedded setup resets `QProgressDialog`'s constructor-armed force-show timer, and the offscreen smoke rejects any progress/partial show or surviving timer. `real-archive-mesh-editor-load-smoke` repeats the read-only PDW PAC transition cold/refresh/warm and fingerprints PAMT/PAZ. The Qt builder retains callback state in hidden sinks and parents its action row inside the side controls so Edit Mesh can hide it. Preview clearing is idempotent after its Qt owner is deleted, and queued state sync drops stale embedded controls so builder-finished cleanup cannot cascade through deleted `QTimer`/`QPushButton` wrappers. Classic remains the per-form default with its fixed Select/Move/Brush/Topology jump bar, inline label/input rows, and viewport-wide status/FPS footer. Its opt-in Bottom Tool Deck keeps the resident viewport under one permanent Win32 parent and reparents only ordinary tool controls into tabbed bottom tools plus a right inspector; its dimensions/tab are session-only and never overwrite classic splitter preferences. Never assign the hidden compact splitters their real panel minimums during construction; apply them only after nonzero client sizing or every preview profile can fail before renderer startup.
- Resident material protocol v2 updates parameters, resources, and bindings in-process. Mesh Editor tracks `editable_imported` and `original_reference` generations independently; Textured settles only after every required role applies and the helper confirms decoded resources have live D3D SRVs. Modify Original leases and reuses the current Archive Browser native package and queues its texture state at open; automatic Archive Browser texture requests defer while Builder is active, while an explicit user refresh remains allowed. Resolved direct DDS bindings publish first, while the full PAC graph follows as a generation/package-correlated fidelity upgrade that cannot overwrite a newer request. Native material inputs carry resolved source-submesh ownership; wrapper indices are not owner aliases. Explicit skin/head-skin evidence blocks decoded packed-channel metal promotion, and detail/damage/grime height remains layer-only. Layered normal synthesis decodes and anchors to the native-selected macro DDS, rejects alternate non-layer wrapper normals, and reconstructs positive tangent Z from BC5 XY before whiteout detail blending. Live and handoff-pending preview-package leases protect every referenced external DDS from native-cache pruning. Accepted package swaps invalidate prior role readiness, fence resource and parameter mutations to the accepted package generation, and republish before restoring a deferred Textured request; failures name the pane while retaining last-valid resources. Initial Archive and authoring packages remain geometry-only. Schema-8 Archive packages use atomic adapter sidecars beside validated native buffers and never enter Python geometry/OBJ/PNG conversion. Renderer-ready native UVs keep the shared Wavefront convention, Source DDS wins over preview PNG, and PAC shader/alpha/two-sided contracts remain independent of DDS resolution. Declared slot/type/subtype/channel evidence outranks path-token guesses; deterministic coverage puts a declared glossiness map below an `ao` parent. Qt byte-decode publication retries never run for authoritative `.dds` inputs because no Qt decoder exists for them; DirectXTex remains the DDS decoder.
