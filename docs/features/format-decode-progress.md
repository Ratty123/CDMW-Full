...
| `.paproj` | 9 | none | none | low | record layout unknown, and four readings are ruled out. The leading u32 differs per file and does not track file size. No header/record-size pair in 8..128 bytes divides any file evenly at the count it declares. Stride analysis finds no fixed record in eight of the nine files -- their strongest period is 4 bytes, which is the noise floor of a float array, not a record. The ninth, projectileshotinfo, does show a 156-byte period from about offset 284 (with 312 confirming it) and carries -1.0 markers at consistent offsets within it, but agreement is only 0.548 against a 0.5 baseline and the fields decode as denormal noise, so the period is real but the layout over it is not established. There is no counter in the header that a candidate grammar can be checked against, which is what made .papr tractable and this so far is not |
