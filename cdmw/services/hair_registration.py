"""Prepare the Damiane barber-registration proof entirely in memory.
...
def prepare_damiane_hair_registration(
    ...
    choices = read_hair_choices(source(DAMIANE_MESH_PARAM))
    ...
    if len(resources) != 1 or not resources[0].lower().endswith("/" + donor.prefab_stem + ".pac"):
        raise HairRegistrationError("The proof requires one unambiguous donor PAC reference.")
    mesh_path = _path(resources[0])
    if not mesh_path.startswith("character/model/1_pc/2_phw/head/hair/"):
        raise HairRegistrationError("The donor is not a Damiane-family hair mesh.")
    ...
    appended = append_hair_choice(source(DAMIANE_MESH_PARAM), template_index=template_index,
    ...
    replacements = (HairRegistrationFile(DAMIANE_MESH_PARAM, appended.data),
