...
NEW_ITEM_STUDIO_SECTION = {
    ...
    <ul>
      ...
      <li><b>Perks and effect</b>: the perks are the Abyss Gear socket items the item carries by default (the tooltip's "Insight I", "Malicebane I" lines); keep the template's or pick up to eight from the whole gem catalogue (no shipped item carries more than four). A weapon effect is one of the shipped effect binaries, grafted into the item's own prefabs as an EffectComponent the way the thrown lightning spear carries its aura; the element presets start from the effects named for weapons and elements, and a grafted fire has been seen drawing in game. An on-hit effect (an explosion when the blade lands) is combat data, not a prefab, and is not offered. <b>Index effects</b> reads every shipped effect once (about a minute, kept on disk); the filter then matches the emitters, textures and meshes an effect is made of, and the panel says what the chosen effect draws and how big its box is at the chosen scale. <b>Place in viewport...</b> opens the resident viewport with the item's mesh as a wire and the effect's box in orange at the current scale and offset; the placement gizmo moves it (Move) or resizes it (Scale), and the numbers come back to the panel. <b>Look</b>: a colour and factors on brightness, particle size, spawn rate and lifetime; with any of them set the effect and the emitters it instances are cloned under stems of the item's own (same length, so nothing in the files moves) and their named values edited in place; which of them the game honours is unproven until the first look.</li>
