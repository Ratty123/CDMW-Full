...
NEW_ITEM_STUDIO_SECTION = {
    ...
    <ul>
      ...
      <li><b>Modelo e icono</b>: conserva el modelo de la plantilla o <b>Importar con el Builder...</b>, que ejecuta Importar malla sobre la malla de la plantilla y entrega el resultado aquí en lugar de escribirlo sobre la plantilla; se reasigna a la familia del objeto nuevo. Los materiales de un modelo importado se escriben por defecto para los shaders PBR simples del juego (SkinnedMeshStandard: albedo, normal y mapa de rugosidad/metal de la fuente, SkinnedMeshEmissive donde la fuente brilla), como las armas del juego guiadas por texturas; desmarca eso para conservar el sidecar en capas del Builder tal como vino. La plantilla es también la base que hereda la importación: sus prefabs de partes deciden qué partes del personaje ocupa el objeto y qué se dibuja a su lado (el pelo de casco de un yelmo), y el panel Plantilla los lista; un yelmo importado sobre el Yelmo de placas del Luchador del Norte deja la cara dibujada, uno sobre el del Guerrero Inquebrantable o el yelmo Canta oculta la cabeza. Dónde queda el modelo lo decide la revisión de colocación del Builder: en las espadas del juego el borde de la guarda del lado de la empuñadura está 0,10 m por delante de la mano (desplazamiento z, + hacia el pomo); un yelmo quiere colocación manual (escala, sin rotación, origen en la cabeza). Conserva el icono o genera uno a partir de una imagen o carpeta, ajustado al formato DDS del icono de la plantilla.</li>
      ...
      <li><b>Ventajas y efecto</b>: las ventajas son los objetos de engarce Abyss Gear que el objeto lleva por defecto (las líneas "Insight I", "Malicebane I" de la descripción); conserva las de la plantilla o elige hasta ocho de todo el catálogo de gemas (ningún objeto del juego lleva más de cuatro). Un efecto de arma es uno de los binarios de efectos del juego, injertado en los prefabs propios del objeto como EffectComponent, igual que la lanza de rayos arrojadiza lleva su aura; los preajustes de elemento parten de los efectos nombrados por armas y elementos, y un fuego injertado se ha visto dibujarse en el juego. Un efecto al golpear (una explosión cuando cae la hoja) es dato de combate, no un prefab, y no se ofrece. <b>Indexar efectos</b> lee cada efecto del juego una vez (cerca de un minuto, guardado en disco); el filtro entonces coincide con los emisores, texturas y mallas de las que está hecho un efecto, y el panel dice qué dibuja el efecto elegido y cómo de grande es su caja a la escala elegida. <b>Colocar en el visor...</b> abre el visor residente con la malla del objeto como alambre y la caja del efecto en naranja a la escala y desplazamiento actuales; el gizmo de colocación la mueve (Mover) o la redimensiona (Escalar), y los números vuelven al panel. <b>Aspecto</b>: un color y factores sobre el brillo, el tamaño de partícula, la tasa de aparición y la vida; con cualquiera de ellos puesto, el efecto y los emisores que usa se clonan bajo stems propios del objeto (de la misma longitud, así que nada se mueve en los archivos) y sus valores con nombre se editan en el sitio; cuáles de ellos respeta el juego no está probado hasta el primer vistazo.</li>
