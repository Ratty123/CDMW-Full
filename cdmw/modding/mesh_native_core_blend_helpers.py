...
from cdmw.modding.mesh_parser import PAC_SKIN_PALETTE_SLOTS
...
def _blend_bone_assignment(
    ...
    if not weights_by_bone:
        return (), ()
    strongest = sorted(weights_by_bone.items(), key=lambda item: (-item[1], item[0]))[:PAC_SKIN_PALETTE_SLOTS]
