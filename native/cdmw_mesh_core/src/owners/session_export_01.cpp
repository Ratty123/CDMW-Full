void mesh_editor_write_export_snapshot_submesh(
    ...
    // only when the caller asked for all three and the contract is valid. A
    // partial request publishes no descriptor at all, so Python can never decode
    // half a contract.
    const std::string vertex_origin_offsets_path = string_or(item.get("vertex_origin_offsets_output_path"), "");
    const std::string vertex_origin_parents_path = string_or(item.get("vertex_origin_parents_output_path"), "");
    const std::string vertex_origin_weights_path = string_or(item.get("vertex_origin_weights_output_path"), "");
    if (submesh.topology_rebuild_valid
        && !vertex_origin_offsets_path.empty()
        && !vertex_origin_parents_path.empty()
        && !vertex_origin_weights_path.empty()
        && mesh_editor_topology_provenance_shape_valid(submesh)) {
        write_int_binary_file(vertex_origin_offsets_path, submesh.vertex_origin_offsets);
        write_int_binary_file(vertex_origin_parents_path, submesh.vertex_origin_parents);
        write_double_binary_file(vertex_origin_weights_path, submesh.vertex_origin_weights);
        exported_submeshes << ",\"vertex_origin_offsets_binary\":";
        write_int_binary_descriptor(exported_submeshes, vertex_origin_offsets_path, submesh.vertex_origin_offsets.size(), 1);
        exported_submeshes << ",\"vertex_origin_parents_binary\":";
        write_int_binary_descriptor(exported_submeshes, vertex_origin_parents_path, submesh.vertex_origin_parents.size(), 1);
        exported_submeshes << ",\"vertex_origin_weights_binary\":";
        write_f64_binary_descriptor(exported_submeshes, vertex_origin_weights_path, submesh.vertex_origin_weights.size());
    }
    exported_submeshes << '}';
}
